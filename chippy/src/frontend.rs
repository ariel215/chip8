...
use chippy_lib::{
    Chip8, Instruction, DISPLAY_COLUMNS, DISPLAY_ROWS, INSTRUCTION_SIZE, MEMORY_SIZE,
};
#[derive(Clone, Copy)]
pub enum KeyInput {
    Chip8Key(u8),
...
impl RaylibDisplay {
    ...
    pub fn load_sound<'a>(&'a mut self) -> Sound<'a> {
        let sound = self
            .raylib_audio
            .new_wave_from_memory("ogg", Self::SOUND_FILE)
            .unwrap();
...
impl Chip8Frontend for RaylibDisplay {
    fn update(&mut self, chip8: &Chip8, show_current_instruction: bool) -> bool {
        ...
        let pixel_width: i32 = ((screen_width / DISPLAY_COLUMNS as i32) as f32
            * (if self.debug_mode {
        ...
        let pixel_height = ((screen_height / DISPLAY_ROWS as i32) as f32
            * (if self.debug_mode {
            ...
            .collect();
        {
            ...
            if self.debug_mode {
                ...
                if show_current_instruction {
                    self.instruction_window.start_addr = max(
                        chip8.pc() - (3 * INSTRUCTION_SIZE),
    ...
    fn get_inputs(&mut self) -> Vec<KeyInput> {
        ...
        {
            inputs.push(KeyInput::Click(vec2!(
...
impl InstructionWindow {
    ...
    pub(crate) fn draw<T: RaylibDraw>(
        ...
        let ram_slice =
            &chip8.memory.ram[self.start_addr..self.start_addr + (self.len * INSTRUCTION_SIZE)];
        let addr_instrs: Vec<(usize, Instruction)> = ram_slice
            .iter()
            .enumerate()
            .tuples()
            .map(|((i1, b1), (_i2, b2)): ((usize, &u8), (usize, &u8))| {
                (self.start_addr + i1, u16::from_be_bytes([*b1, *b2]).into())
            })
            .collect();
