...
use chippy_lib::{Chip8, DISPLAY_COLUMNS, DISPLAY_ROWS, MEMORY_SIZE};

use bitvec::BitArr;
use egui::{
    ahash::HashMap, pos2, vec2, Color32, FontId, Key, Layout, Rect, Response, CornerRadius, Sense, Ui
};
...
impl EguiDisplay {
    ...
    fn is_breakpoint(&self, addr: usize) -> bool {
        match self.instruction_window.breakpoints.get(addr) {
            Some(val) => *val,
            None => false
        }
    }
