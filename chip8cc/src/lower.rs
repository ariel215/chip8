...
use crate::parser::{ParseNode, Operator};
...
impl<'a> LoweringVisitor<'a> {
    ...
    fn get_or_create_register(&mut self, variable: &'a str) -> Value{
        ...
        if let Some(reg) = self.variables.get(variable){
            ...
            Value::Reg(*self.variables.entry(variable).or_insert(Register(next_index as u8)))
        }
    }
                ...
                match operation.operator {
                    ...
                    Operator::Plus =>  {
                        ...
                        Value::Number(n) => {
                            ...
                            self.instructions.push(Instruction::SetImm(place.value(), n));
                            place
                            }, 
