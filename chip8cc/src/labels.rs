...
struct BasicBlock{
    ...
    /// A basic block is the equivalent of block.start..=block.end
    end: InstructionHandle
}

struct CFG {
    entry: BlockHandle,
    edges: BTreeMap<BlockHandle, Vec<BlockHandle>>,
}
    ...
    pub fn fix_references(&mut self){
        ...
        for (usage, label) in self.references.iter().enumerate(){
            if let Some(label) = *label{
                ...
                dbg!(target);
                let sizes = &self.instructions[..target].iter().map(
                    |l| match l {
                ...
                let new_instruction = match self.instructions[usage] {
                    ...
                    _ => panic!("unexpected instruction: {:?} at {:?}", self.instructions[usage], usage)
                };
...
fn error<'a, T>(rule: &Pair<'a, Rule>, message: &str) -> Result<T, Error>{
    return Err(Error::new_from_pos(
        ErrorVariant::CustomError { message: format!("{}: {:?}", message, rule) }, 
...
fn parse_load<'a>(load_args: Pair<'a, Rule>) -> ParseResult<'a>{
    ...
    Ok((Line::Instr(match pair.as_rule() {
        Rule::ldchar => {
