...
pub(crate) fn do_instruction(memory: &mut Memory, registers: &mut Registers) {
    ...
    match instruction {
        ...
        Instruction::SubReg(vx, vy) => {
            let (result, flag) =
                subtract_with_underflow(registers.vn[vx as usize], registers.vn[vy as usize]);
            registers.vn[vx as usize] = result;
            registers.vn[15] = flag as u8;
        }
        ...
        Instruction::SkipKeyPressed(reg) => {
            if memory.keys[registers.vn[reg as usize] as usize] {
                registers.pc += INSTRUCTION_SIZE
            }
        }
        ...
        Instruction::RegDump(reg) => memory.ram[registers.i..(registers.i + reg as usize) + 1]
            .copy_from_slice(&registers.vn[0..(reg + 1) as usize]),
        Instruction::RegLoad(vx) => registers.vn[0..vx as usize + 1]
            .copy_from_slice(&memory.ram[registers.i..registers.i + vx as usize + 1]),
    }
    if !matches!(
        instruction,
        Instruction::Jump(_) | Instruction::JumpOffset(_) | Instruction::Call(_)
    ) {
        registers.pc += INSTRUCTION_SIZE;
    }
}
...
fn test_instructions() {
    assert_eq!(
        <u16 as Into<Instruction>>::into(0x00E0_u16),
        Instruction::ClearScreen
    );
    assert_eq!(
        <u16 as Into<Instruction>>::into(0x1e35_u16),
        Instruction::Jump(0xe35)
    );
    assert_eq!(
        <u16 as Into<Instruction>>::into(0x5e30_u16),
        Instruction::SkipEqReg(0xe, 0x3)
    );
}
...
fn test_instruction_in_memory() {
    ...
    registers.pc = 1024;
    assert_eq!(
        get_instruction(&memory, &registers),
        Instruction::ClearScreen
    );
    registers.pc += INSTRUCTION_SIZE;
    assert_eq!(
...
#[test]
fn test_load_char() {
    let instrs: Vec<u16> = ["ld v0 1", "ld v1 0", "ld v2 0", "ld f 0", "drw 1 2 5"]
        .into_iter()
        .map(|s| Instruction::from_mnemonic(s).unwrap().into())
        .collect();
    ...
    assert!(memory.display[[1, 1]]);
    assert!(memory.display[[2, 1]]); // x**x
