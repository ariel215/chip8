...
fn assemble(input: ClioPath, output: ClioPath) {
    ...
    if let Ok(ref mut input) = input.open() {
        ...
        match to_binary(&text) {
            Ok(bytes) => {
                let bytes: Vec<_> = bytes.into_iter().flat_map(u16::to_be_bytes).collect();
                let mut output = output
                    .create()
                    .expect(&format!("Could not create file {}", output_name));
                output
            ...
            Err(error) => {
                eprintln!(
                    "Error in assembly file {}:{}",
                    input_name,
                    error.line_number + 1
                );
                eprintln!(
                    "Could not parse '{}' : {}",
                    error.error.mnemonic, error.error.message
                );
            }
        }
    } else {
    ...
    for (line_number, line) in text.lines().enumerate() {
        ...
        for mnemonic in mnemonics {
            ...
            match Instruction::from_mnemonic(mnemonic.trim()) {
                ...
                Err(error) => return Err(ParsingError { error, line_number }),
            }
        }
    }
    Ok(bytes)
